...
Conversor de monedas sobre la API de [freecurrencyapi.com](https://freecurrencyapi.com): permite listar
las monedas soportadas, consultar cotizaciones y convertir un monto a una o varias monedas, tanto con
la cotización actual como con la de una fecha pasada.
...
Verificá que `JAVA_HOME` apunte a un JDK 25:
...
> falla con el mensaje `Build requires Java 25`. Instalá el 25 y exportá `JAVA_HOME`:
>
> ```bash
...
sin llegar a hacer ninguna llamada a la API:

```
Falta FREECURRENCYAPI_KEY. Copia .env.example a .env y completa tu key de freecurrencyapi.com.
```

Como alternativa, la key también se toma de una variable de entorno con el mismo nombre, útil para CI
o si preferís no usar el archivo:
...
Compila, corre los tests y valida la cobertura. El build **falla si la cobertura baja del 100%**
en líneas y ramas (`jacoco:check`), con `Main` como única clase excluida por ser el composition root.
...
nada de `client/`, `http/` ni `parser/`**. La única flecha entre capas va del detalle al negocio
(`CurrencyApiProvider` implementa los puertos que define `service/`).

```
edu.itba.class1.exchange
├── model/                    Value objects del dominio
│   ├── MoneyAmount           Monto + moneda, redondea según ISO 4217
│   ├── CurrencyRate          Cotización con timestamp
│   └── Conversion            Monto convertido + cotización usada
├── service/                  Reglas de negocio y puertos de salida
│   ├── CurrencyConverter             Conversiones con cotización actual
│   ├── HistoricalCurrencyConverter   Conversiones con cotización histórica
│   ├── CurrencyRateProvider          Puerto: cotizaciones
│   ├── HistoricalCurrencyRateProvider Puerto: cotizaciones históricas
│   ├── CurrencyCatalog               Puerto: catálogo de monedas
│   └── error/                        Excepciones de dominio
├── client/currencyapi/       Adaptador de freecurrencyapi
│   ├── CurrencyApiProvider   Implementa los puertos, traduce DTO → dominio
│   ├── CurrencyApiClient     Puerto de integración
│   ├── FreeCurrencyApiClient Implementación concreta
│   └── response/             DTOs de la API
├── client/error/             Excepciones de integración
├── http/                     Cliente HTTP y mapeo de status → excepción
...
| 3 | Solo la cotización entre dos monedas | `CurrencyRateProvider.getCurrencyRate()` |
...
sella con el instante en que se obtuvo. Las históricas llevan la fecha solicitada, no la de la consulta.
...
Aparte, `HttpTransportException` cubre las fallas de conexión (timeout, DNS, host inalcanzable) y
`RateNotAvailableException` el caso de que la API responda OK pero sin la cotización pedida.
